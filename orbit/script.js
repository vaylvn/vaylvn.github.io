...
function renderShareCard(score) {
  ...
  const scoreText = (isMobile) ? toString(score) + "ᵐ" : toString(score);
...
function drawStaticRing(ctx) {
  ...
  // Angles (static, cosmetic)
  const lockStart = Math.PI * 0.75;
  const lockSize  = Math.PI * 0.18;
  const orbAngle  = Math.PI * 1.9;
  ...
  // --- Lock segment ---
  ctx.beginPath();
  ctx.arc(cx, cy, radius, lockStart, lockStart + lockSize);
  ctx.strokeStyle = "#f2c94c"; // Numi yellow
  ctx.lineWidth = ringWidth;
  ctx.lineCap = "round";
  ctx.stroke();
