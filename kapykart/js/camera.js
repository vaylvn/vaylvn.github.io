...
//
// A later pass tried pushing this 400->550, reasoning that main.js's floor
// canvas no longer scaling its backing store by devicePixelRatio had freed
// up real budget to spend on more reach. That reasoning had an unverified
// assumption baked in: it assumed a HiDPI screen (dpr>1), where removing
// the dpr multiplier actually shrinks the canvas's real pixel count.
// Checked directly against the actual streamer machine this runs on:
// window.devicePixelRatio is 1 there, meaning that "freed budget" never
// existed for this setup - the dpr fix was a no-op (multiplying by 1
// changes nothing), and the subsequent buffer growth was pure added
// compositing cost with no offsetting saving, which is exactly what caused
// framerates to tank again after adding a third track. Reverted back to
// 400. The dpr-decoupling fix in main.js itself is still correct and worth
// keeping (harmless here, a real win on any HiDPI viewer), but it turns out
// the actual dominant cost on THIS hardware is simpler and blunter than
// that: this canvas is already FLOOR_OVERSIZE_WIDTH x FLOOR_OVERSIZE_HEIGHT
// times the viewport's own area (over 40x, even before any dpr math), and
// compositing a CSS 3D transform (perspective+rotateX) on an element that
// size is expensive on its own, independent of DPI. The only lever that
// actually reduces THAT cost is shrinking these oversize constants
// themselves - which directly re-opens the cutoff trade-off. There isn't a
// free win left here; growing forward reach again means either accepting
// worse framerates on this machine, or a genuinely different rendering
// approach (e.g. avoiding one giant CSS-transformed canvas entirely -
// worth a real design discussion, not a quick constant tweak).
...
//
// A later pass tried HEIGHT 5.862->7.931 alongside FOLLOW_AHEAD_WORLD_UNITS's
// 400->550, reasoning there was freed dpr-related budget to spend (see that
// constant's own comment for the full story - the premise didn't hold on
// the actual streamer machine this runs on, which reported
// devicePixelRatio=1, so that fix never freed anything here). Reverted
// back to 5.862/400 - real, not just algebraic: the floor's forward reach
// in world-units is exactly FLOOR_OVERSIZE_HEIGHT * FOLLOW_FORWARD_FRACTION
// * (track.def.width / FOLLOW_FIT_FACTOR) (solve followFloorProject's `y =
// floorHeight*fraction - ry*scale` for ry at y=0, then substitute
// floorHeight = viewportHeight*FLOOR_OVERSIZE_HEIGHT and scale =
// (FIT_FACTOR/trackWidth) * min(viewportW,viewportH) - viewportHeight
// cancels with min(viewportW,viewportH) for any landscape aspect ratio,
// since height is the smaller dimension, leaving a viewport-independent
// constant), verified directly against the real followFloorProject
// function (not just algebra) to resolve to ~414 real world-units at
// 5.862 - i.e. 400's nominal label already carries a small built-in
// margin, same as before this whole round-trip.
